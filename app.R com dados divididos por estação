...
# Leitura dos dados
stations_names_code <- read.csv("nomes_codigos_estacoes.csv", sep=",", header = TRUE)
est_nomes <- stations_names_code

# Lista de todas as estações no diretório
arquivos_csv <- list.files(pattern = "^[ABFS].*\\.csv$", full.names = TRUE)

# Lista para armazenar os data frames
lista_dataframes <- list()

# Loop para ler os arquivos CSV
for (arquivo in arquivos_csv) {
  # Nome do arquivo sem a extensão
  nome_arquivo <- tools::file_path_sans_ext(basename(arquivo))
  assign(nome_arquivo, read.csv(arquivo))
  lista_dataframes[[nome_arquivo]] <- get(nome_arquivo)
}

# Loop para iterar sobre os data frames
for (df_nome in names(lista_dataframes)) {
  df <- lista_dataframes[[df_nome]]
  
  # Converte a coluna "Date" para o formato Date
  df$Date <- as.Date(df$Date)
  
  # Atualiza o data frame na lista
  lista_dataframes[[df_nome]] <- df
}
...
server <- function(input, output){
  ...
  output$graph_media_movel <- renderPlot({
    ...
    dados = lista_dataframes[[estacao]]
